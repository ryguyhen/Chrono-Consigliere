...
// Extracted outside the parent so React can stable-identity this component
// across renders and not tear down / rebuild the DOM on every filter change.
...
export function BrowseFilters({ brands, styles, movements, conditions, dealers }: BrowseFiltersProps) {
  ...
  const isActive = useCallback((key: string, value: string) => params.getAll(key).includes(value), [params]);
  ...
  const sharedSectionProps = { isActive, onToggle: toggleParam };

  const filterContent = (
    <>
      <FilterSection title="Brand" items={brands} paramKey="brand" {...sharedSectionProps} />
      <FilterSection title="Style" items={styles} paramKey="style" labelMap={STYLE_LABELS} {...sharedSectionProps} />
      <div className="mb-6">
      ...
      <FilterSection title="Movement" items={movements} paramKey="movement" labelMap={MOVEMENT_LABELS} {...sharedSectionProps} />
      <FilterSection title="Condition" items={conditions} paramKey="condition" labelMap={CONDITION_LABELS} {...sharedSectionProps} />
      <FilterSection title="Dealer" items={dealers} paramKey="dealer" {...sharedSectionProps} />
    </>
  );
    ...
    <>
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-[60] flex justify-end">
          ...
          />
          <div
            className="relative w-[300px] max-w-[85vw] h-full bg-surface overflow-y-auto px-5 pt-6"
